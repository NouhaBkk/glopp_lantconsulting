...
const AdminDashboard = () => {
  ...
  const handleUpdateOffer = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/offers/${editingOffer.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteOffer = async (id) => {
    if (window.confirm('Voulez-vous vraiment supprimer cette offre ?')) {
      try {
        await fetch(`http://localhost:8080/api/offers/${id}`, { method: 'DELETE' });
  ...
  return (
    //<div className="admin-dashboard-container" style={{ background: `url(${coverImage}) no-repeat center center/cover` }}>
