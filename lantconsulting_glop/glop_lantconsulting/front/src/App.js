...
import Banner from './components/Banner'; // adapte le chemin si nécessaire
...
function App() {
  ...
  const handleSubscribe = async (offerId) => {
    ...
    try {
      const response = await fetch('http://backend/api/contracts/subscribe', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          offerId,
        ...
        <div className="main-content">
          <Routes>
            <Route
              ...
              element={
                user ? (
                  user.roles.includes("ROLE_ADMIN") ? <AdminDashboard /> : <EspacePersonnel />
                ) : (
            ...
            <Route path="/espacepersonnel" element={user?.roles.includes('ROLE_USER') ? <EspacePersonnel accountname={user?.accountname} /> : <Navigate to="/admin" />} />
